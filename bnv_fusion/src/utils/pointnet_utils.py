...
class SimpleMLP(nn.Module):
    def __init__(self, n_input_dims, n_output_dims, network_config):
        ...
        # Add output layer
        layers.append(nn.Linear(input_dim, n_output_dims))
...
class IdentityEncoding(nn.Module):
    def __init__(self, scale=1.0, offset=0.0):
        super(IdentityEncoding, self).__init__()
        self.scale = scale
        self.offset = offset
...
class tcnnPointNetEncoder(nn.Module):
    def __init__(self, feat_dims, in_channels, **kwargs):
        ...
        # self.encoding = IdentityEncoding(scale=config["encoding"]["scale"], offset=config["encoding"]["offset"])
        # self.network = SimpleMLP(n_input_dims=in_channels, n_output_dims=feat_dims, network_config=config["network"])
        self.feat_dims = feat_dims
