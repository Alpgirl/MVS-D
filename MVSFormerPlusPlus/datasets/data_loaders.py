...
class Sk3DLoader(DataLoader):

    def __init__(self, data_path, data_list, mode, num_srcs, num_depths, interval_scale=1.0,
                 shuffle=True, batch_size=1, fix_res=False, max_h=None, max_w=None,
                 dataset_eval='sk3d', refine=True, num_workers=4, random_crop=False, 
                 iterative=False, augment=False, aug_args=None,
                 height=None, width=None, data_set_type='stage4', **kwargs):
        if mode in ['train', 'val']:
            ...
            self.mvs_dataset = Sk3DDataset(data_path, data_list, mode, num_srcs, num_depths, interval_scale,
                                          shuffle=shuffle, batch_size=batch_size,
                                          max_h=max_h, max_w=max_w, fix_res=fix_res, dataset_eval=dataset_eval, refine=refine,
                                          iterative=iterative, **kwargs)
